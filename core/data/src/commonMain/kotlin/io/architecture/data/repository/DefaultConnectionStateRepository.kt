...
class DefaultConnectionStateRepository(
    ...
    override fun streamRoundTripTime(interval: Duration): Flow<UpstreamRtt> =
        network.streamServerTime()
            ...
            .map { serverTime -> UpstreamRtt(Clock.System.now().toEpochMilliseconds() - serverTime.clientSentTime) }
    ...
    private suspend fun sendClientTimeWith(interval: Duration) {
        delay(interval)
        try {
            network.sendClientTime(NetworkClientTime(Clock.System.now().toEpochMilliseconds())) // TODO Look up deatils
